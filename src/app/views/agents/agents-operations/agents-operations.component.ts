...
export class AgentsOperationsComponent implements OnInit {
  ...
  getAllClientsForSelse() {
    ...
    this.spinner.show();
    this.viewService
      .ShowById("assign-agent-customer/sales-client", this.salesId)
      .then((res) => {
        this.spinner.hide();
        this.clientsSalesList = res["data"].reverse();
  ...
  unAssignClientFromSales() {
    ...
    if (this.selectedCustomersList.length == 0) {
      ...
      this.viewService
        .UnAssignCustomersToSales(this.selectedCustomersList)
        .then((res) => {
          this.toastr.success("تم الحذف بنجاح");
          window.location.reload();
          this.clientsSalesList = this.clientsSalesList.filter((x) =>
