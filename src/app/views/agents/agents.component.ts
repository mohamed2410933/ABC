...
export class AgentsComponent implements OnInit {
  ...
  getAllSales(page, q = "") {
    this.spinner.show();
    this.viewService
      .GetAll("assign-agent-customer/sales", page, q)
      .then((res) => {
        this.spinner.hide();
  ...
  assignCustomersToSales() {
    ...
    this.spinner.show();
    this.viewService
      .AssignCustomersToSales(this.selectedCustomersList, this.salesId)
      .then((res) => {
        this.spinner.hide();
        this.toastr.success(res["message"]);
        window.location.reload();
        this.agentsList = this.agentsList.filter((x) =>
          this.selectedCustomersList.find((e) => x.id !== e)
        );
  ...
  onActivate(event) {
    if (event.type == "checkbox") {
      if (this.selectedCustomersList.includes(event.row.id)) {
        this.selectedCustomersList = this.selectedCustomersList.filter(
          (e) => e !== event.row.id
        );
      } else {
  ...
  getAllMembers(page, q = "") {
    this.spinner.show();
    this.viewService
      .GetAll("assign-agent-customer/admin-agents", page, q)
      .then((res) => {
        this.spinner.hide();
