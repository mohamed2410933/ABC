...
export class ReportsOperationsComponent implements OnInit {
  ...
  addEditReport(id) {
    ...
    this.viewService.addEditReports(this.addReportForm.value, +id).then(res => {
      this.spinner.hide();
      this.toastr.clear();
  ...
  getReportById(id) {
    this.viewService.getReportById(id).then(res => {
      // this.addReportForm.get('type').setValue(res['data']['name']);
      this.addReportForm.get('discription').setValue(res['data']['description']);
